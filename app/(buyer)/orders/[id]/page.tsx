...
import { advanceOrderAction } from "@/app/(buyer)/orders/actions";
...
export default async function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const canAdvance = activeKey === "payment" || activeKey === "delivered";
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-6 md:py-8">
      ...
      {isOwner && !isDelivered && (activeKey === "production" || activeKey === "shipped") && (
        <div className="mt-2 rounded-2xl border border-line p-4 text-[12.5px] text-sub">
          {activeKey === "production"
            ? "Your supplier is preparing this order."
            : "Your supplier has shipped this order."}
        </div>
      )}

      {isOwner && !isDelivered && canAdvance && (
        <div className="mt-2 rounded-2xl border border-line p-4">
          <p className="text-[12px] text-sub">
            <b className="text-ink">Internal control, not for production.</b>{" "}
            {activeKey === "payment"
              ? "Stands in for Razorpay's payment webhook until that exists."
              : "Confirms you've received the order."}
          </p>
          <form action={advanceOrderAction} className="mt-3">
            ...
            <SubmitButton pendingText="Advancing…" className={buttonClassName({ variant: "outline", size: "sm" })}>
              {activeKey === "payment" ? "Mark payment received" : "Confirm delivery"}
            </SubmitButton>
          </form>
        </div>
      )}
